<?php
/**
 * Debug für echte Live-Genehmigung - simuliert exakt den echten POST-Request
 */
...
echo "<h1>Debug: Echte Live-Genehmigung</h1>";
...
// 3. Simuliere echten POST-Request wie in admin/reservations.php
echo "<h2>3. Simuliere echten POST-Request</h2>";
...
try {
    ...
    $stmt->execute([$vehicle_id, 'Live Test', 'live@test.com', 'Live Test für echte Genehmigung', $test_start, $test_end]);
            ...
            try {
                ...
                if ($reservation) {
                    ...
                    if (function_exists('create_google_calendar_event')) {
                        ...
                        error_log("LIVE DEBUG: Starte Google Calendar Event Erstellung für Reservierung #$reservation_id");
                        error_log("LIVE DEBUG: Parameter - Fahrzeug: " . $reservation['vehicle_name'] . ", Grund: " . $reservation['reason']);
                        error_log("LIVE DEBUG: Parameter - Start: " . $reservation['start_datetime'] . ", Ende: " . $reservation['end_datetime']);
                        error_log("LIVE DEBUG: Parameter - Reservation ID: " . $reservation['id']);
                        ...
                        error_log("LIVE DEBUG: Google Calendar Event Erstellung abgeschlossen. Event ID: " . ($event_id ?: 'NULL'));
...
foreach ($log_files as $log_file) {
    if ($log_file && file_exists($log_file)) {
        ...
        foreach ($lines as $line) {
            if (strpos($line, 'Google Calendar') !== false || strpos($line, 'create_google_calendar_event') !== false || strpos($line, 'LIVE DEBUG:') !== false) {
