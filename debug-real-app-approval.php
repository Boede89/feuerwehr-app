<?php
/**
 * Debug für echte App-Genehmigung mit exakter Simulation
 */
...
// 3. Simuliere EXAKT die admin/reservations.php Logik
echo "<h2>3. Simuliere EXAKT die admin/reservations.php Logik</h2>";
...
try {
    ...
    $stmt->execute([$vehicle_id, 'Real App Test', 'realapp@test.com', 'Real App Test für echte Genehmigung', $test_start, $test_end]);
    ...
    // 1. Reservierung genehmigen (EXAKT wie in admin/reservations.php)
    ...
    echo "<h3>Google Calendar Event erstellen (EXAKT wie in admin/reservations.php)</h3>";
    ...
    try {
        ...
        if ($reservation) {
            ...
            if (function_exists('create_google_calendar_event')) {
                ...
                error_log("REAL APP DEBUG: Starte Google Calendar Event Erstellung für Reservierung #$reservation_id");
                error_log("REAL APP DEBUG: Parameter - Fahrzeug: " . $reservation['vehicle_name'] . ", Grund: " . $reservation['reason']);
                error_log("REAL APP DEBUG: Parameter - Start: " . $reservation['start_datetime'] . ", Ende: " . $reservation['end_datetime']);
                error_log("REAL APP DEBUG: Parameter - Reservation ID: " . $reservation['id']);
                ...
                error_log("REAL APP DEBUG: Google Calendar Event Erstellung abgeschlossen. Event ID: " . ($event_id ?: 'NULL'));
...
foreach ($log_files as $log_file) {
    if ($log_file && file_exists($log_file)) {
        ...
        foreach ($lines as $line) {
            if (strpos($line, 'Google Calendar') !== false || strpos($line, 'create_google_calendar_event') !== false || strpos($line, 'REAL APP DEBUG:') !== false) {
