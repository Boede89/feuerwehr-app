<?php
/**
 * Test für echte admin/reservations.php direkt
 */
...
echo "<h1>Test: Echte admin/reservations.php direkt</h1>";
...
// 2. Simuliere admin/reservations.php Umgebung
echo "<h2>2. Simuliere admin/reservations.php Umgebung</h2>";
...
try {
    ...
    if ($vehicle) {
        ...
        $stmt->execute([$vehicle['id'], 'Admin Direct Test', 'admindirect@test.com', 'Admin Direct Test für Google Calendar', $test_start, $test_end]);
        ...
        error_log("ADMIN RESERVATIONS: Google Calendar: Versuche Event für Reservierung #$test_reservation_id zu erstellen");
        error_log("ADMIN RESERVATIONS: Parameter - Fahrzeug: " . $vehicle['name'] . ", Grund: Admin Direct Test für Google Calendar");
        error_log("ADMIN RESERVATIONS: Parameter - Start: " . $test_start . ", Ende: " . $test_end);
        error_log("ADMIN RESERVATIONS: Parameter - Reservation ID: " . $test_reservation_id);
        
        $event_id = create_google_calendar_event(
            $vehicle['name'],
            'Admin Direct Test für Google Calendar',
            $test_start,
            $test_end,
            $test_reservation_id
        );
...
foreach ($log_files as $log_file) {
    if ($log_file && file_exists($log_file)) {
        ...
        foreach ($lines as $line) {
            if (strpos($line, 'Google Calendar') !== false || strpos($line, 'create_google_calendar_event') !== false || strpos($line, 'ADMIN RESERVATIONS:') !== false) {
