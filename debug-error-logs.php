...
// 2. Prüfe Error Logs
echo "<h2>2. Error Logs prüfen</h2>";
...
foreach ($log_files as $log_file) {
    if ($log_file && file_exists($log_file)) {
        ...
        if (!empty($google_calendar_errors)) {
            ...
            foreach (array_slice($google_calendar_errors, -20) as $error) {
...
// 3. Teste Google Calendar Integration mit detailliertem Logging
echo "<h2>3. Teste Google Calendar Integration mit detailliertem Logging</h2>";
...
if (function_exists('create_google_calendar_event')) {
    ...
    try {
        ...
        if ($vehicle) {
            ...
            $stmt->execute([$vehicle['id'], 'Error Log Test', 'errorlog@test.com', 'Error Log Test für Google Calendar', $test_start, $test_end]);
            ...
            $event_id = create_google_calendar_event(
                $vehicle['name'],
                'Error Log Test für Google Calendar',
                $test_start,
                $test_end,
                $test_reservation_id
            );
...
// 4. Prüfe Error Logs erneut
echo "<h2>4. Error Logs nach Test prüfen</h2>";
...
foreach ($log_files as $log_file) {
    if ($log_file && file_exists($log_file)) {
        ...
        $lines = explode("\n", $log_content);
        foreach ($lines as $line) {
            if (strpos($line, 'Google Calendar') !== false || strpos($line, 'create_google_calendar_event') !== false || strpos($line, 'Error Log Test') !== false) {
                $google_calendar_errors[] = $line;
            }
        }
