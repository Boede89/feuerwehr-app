<?php
/**
 * Hilfsfunktion: Mängelbericht-PDF generieren (ohne exit, für Kombi-Versand).
 * @return string|null PDF-Inhalt oder null bei Fehler
 */
function maengelbericht_generate_pdf_content($db, $id) {
    $id = (int)$id;
    if ($id <= 0) return null;
    $bericht = null;
    try {
        $stmt = $db->prepare("SELECT m.*, COALESCE(u.first_name, '') AS user_first_name, COALESCE(u.last_name, '') AS user_last_name FROM maengelberichte m LEFT JOIN users u ON u.id = m.user_id WHERE m.id = ?");
        $stmt->execute([$id]);
