<?php
/**
 * Gerätewartmitteilung server-seitig drucken.
 */
session_start();
...
if ($alle || $ids !== '') {
    ...
    if ($ids !== '') $_GET['ids'] = $ids;
    require __DIR__ . '/geraetewartmitteilung-pdf-alle.php';
} else {
    $_GET['_return'] = '1';
    $_GET['id'] = $id;
...
$pdfPath = tempnam(sys_get_temp_dir(), 'gwm_print_') . '.pdf';
...
if ($return_var !== 0) {
    $err = implode(' ', $output);
    error_log('print-geraetewartmitteilung lp error: ' . $err);
