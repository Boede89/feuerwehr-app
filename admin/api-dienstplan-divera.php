 ...
 * Verwendet ausschließlich den Einheits-Key (einheit_settings), kein Fallback auf Benutzer-Key.
 */
session_start();
...
if ($effective_einheit_id <= 0) {
    $effective_einheit_id = isset($input['einheit_id']) ? (int)$input['einheit_id'] : null;
}
if ($effective_einheit_id <= 0 && function_exists('get_admin_einheit_filter')) {
    $f = get_admin_einheit_filter();
    $effective_einheit_id = $f ? (int)$f : null;
}
if ($effective_einheit_id <= 0 && function_exists('get_current_einheit_id')) {
...
// POST: Import oder Export (input wurde oben bereits gelesen)
