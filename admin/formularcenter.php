...
try {
    ...
    if ($filter_typ !== '') {
        if ($filter_typ === 'einsatz') {
            ...
            // Entspricht get_anwesenheitsliste_typ_label() === 'Übungsdienst' (nicht Einsatz, nicht Sonstiges)
            $sql .= " AND NOT (a.typ = 'einsatz')
                AND NOT (a.typ = 'dienst' AND COALESCE(d.typ, '') = 'einsatz')
                AND (
                    (a.typ = 'dienst' AND COALESCE(d.typ, '') != 'sonstiges')
                    OR (a.typ = 'manuell' AND (a.bezeichnung IS NULL OR TRIM(a.bezeichnung) != 'Sonstiges')
                        AND NOT (a.custom_data IS NOT NULL AND TRIM(COALESCE(JSON_UNQUOTE(JSON_EXTRACT(a.custom_data, '$.typ_sonstige')), '')) = 'sonstiges'))
                    OR (a.typ NOT IN ('einsatz', 'dienst', 'manuell'))
                )";
        ...
        function handlePrintResponse(data, btn, btnText) {
            if (data.success) {
                if (data.open_pdf && data.pdf_base64) {
                    ...
                    var w = window.open(url, '_blank', 'noopener,width=900,height=700');
                    if (w) {
                        w.onload = function() {
                            try { w.print(); } catch (e) {}
                        ...
                        newWinBtn.onclick = function() { var w2 = window.open(url, '_blank', 'noopener,width=900,height=700'); if (w2) w2.onload = function() { try { w2.print(); } catch (e) {} }; };
                        var closeBtn = document.createElement('button');
                        closeBtn.textContent = 'Schließen';
                        closeBtn.className = 'btn btn-primary';
                        closeBtn.onclick = function() { iframe.remove(); btnBar.remove(); URL.revokeObjectURL(url); };
                    ...
                    showPrintToast('PDF wurde geöffnet. Sie können lokal drucken (Strg+P).', true);
                } else {
