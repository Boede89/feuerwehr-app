<?php
/**
 * Geräte pro Fahrzeug verwalten (Einstellungen)
 */
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if ($action === 'add') {
            ...
            if ($name !== '') {
                try {
                    $stmt = $db->prepare("INSERT INTO vehicle_equipment (vehicle_id, name, sort_order) VALUES (?, ?, 0)");
...
$equipment = [];
try {
    $stmt = $db->prepare("SELECT id, name, sort_order FROM vehicle_equipment WHERE vehicle_id = ? ORDER BY sort_order, name");
    $stmt->execute([$vehicle_id]);
