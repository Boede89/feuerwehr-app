...
foreach (array_merge(glob($root . '/admin/debug-*.php') ?: [], glob($root . '/admin/test-*.php') ?: []) as $path) {
    $name = basename($path);
    if ($name === 'debug-scripts.php') continue; // Übersichtsseite selbst nicht verlinken
...
$labels = [
    ...
    'debug-atemschutz-liste.php' => 'Atemschutz-Liste prüfen',
    'debug-calendar-conflicts.php' => 'Kalender-Konflikte prüfen',
    ...
    'test-email.php' => 'Test-E-Mail senden',
    'test-member-courses.php' => 'Mitglieder-Kurse',
    'test-reservation-api.php' => 'Reservierungs-API (process-reservation)',
];
...
$categories = [
    'Reservierung & Dashboard' => ['debug-admin-reservations', 'debug-dashboard-', 'test-reservation'],
    'E-Mail' => ['debug-email-', 'test-email'],
    'Google Calendar' => ['debug-google-calendar-', 'debug-calendar-', 'debug-create-google'],
    'Datenbank' => ['debug-database', 'debug-db', 'debug-logs'],
    'Divera & Admin' => ['debug-divera', 'debug-room-', 'test-member'],
    'Atemschutz' => ['debug-atemschutz-', 'test-atemschutz-'],
    'Sonstige' => []
];
