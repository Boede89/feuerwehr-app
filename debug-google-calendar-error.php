<?php
/**
 * Debug für Google Calendar Fehler in der echten App
 */
...
echo "<h1>Debug: Google Calendar Fehler in der echten App</h1>";
...
// 2. Teste Google Calendar Integration mit detailliertem Logging
echo "<h2>2. Google Calendar Integration mit detailliertem Logging testen</h2>";
...
if (function_exists('create_google_calendar_event')) {
    ...
    try {
        ...
        if ($vehicle) {
            ...
            $stmt->execute([$vehicle['id'], 'Error Debug Test', 'error@test.com', 'Error Debug Test für Google Calendar', $test_start, $test_end]);
            ...
            $event_id = create_google_calendar_event(
                $vehicle['name'],
                'Error Debug Test für Google Calendar',
                $test_start,
                $test_end,
                $test_reservation_id
            );
...
foreach ($log_files as $log_file) {
    if ($log_file && file_exists($log_file)) {
        ...
        foreach ($lines as $line) {
            if (strpos($line, 'Google Calendar') !== false || strpos($line, 'create_google_calendar_event') !== false || strpos($line, 'Error Debug') !== false) {
...
echo "<h2>4. Google Calendar Service Account direkt testen</h2>";
...
try {
    ...
    if (!empty($service_account_json)) {
        ...
        echo "<p><strong>Teste direkten Service Account Aufruf...</strong></p>";
        
        $event_id = $google_calendar->createEvent(
            'Direct Test Event',
            $test_start,
            $test_end,
            'Direct Test Event für Error Debug'
        );
        
        if ($event_id) {
            echo "<p style='color: green;'>✅ Direkter Service Account Aufruf erfolgreich! Event ID: " . htmlspecialchars($event_id) . "</p>";
            
            // Event löschen
            $google_calendar->deleteEvent($event_id);
            echo "<p>✅ Test Event gelöscht</p>";
        } else {
            echo "<p style='color: red;'>❌ Direkter Service Account Aufruf fehlgeschlagen</p>";
        }
    } else {
        echo "<p style='color: red;'>❌ Service Account JSON nicht konfiguriert</p>";
    }
} catch (Exception $e) {
    echo "<p style='color: red;'>❌ Fehler beim direkten Service Account Test: " . htmlspecialchars($e->getMessage()) . "</p>";
    echo "<p><strong>Stack Trace:</strong></p>";
    echo "<pre>" . htmlspecialchars($e->getTraceAsString()) . "</pre>";
}
